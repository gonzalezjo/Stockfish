...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!is_loss(bestValue))
        {
            ...
            int moveLimit = std::max(1, 2 - (tailBudget > 2048));
                ...
                && move.type_of() != PROMOTION)
            {
                ...
                Value futilityValue =
                  futilityBase + tailBudget / 48 + PieceValue[pos.piece_on(move.to_sq())];
                ...
                if (!pos.see_ge(move, alpha - futilityBase + tailBudget / 64))
                {
            ...
            if (!pos.see_ge(move, -73 + tailBudget / 128))
                continue;
        }
        ...
        qTailBudgetHistory[ss->ttHit][move.to_sq() == prevSq][moveBand(moveCount)]
          ...
          << (value > alphaBeforeMove ? -1152 : 736);
